...
export default function DashboardPage() {
        ...
        prev.map((a) =>
          a._id === data.agentId || a._id === data.agentId?.toString()
            ? {
                ...
                onSiteTicket: data.ticketDbId
                  ? { ticketId: data.ticketId, _id: data.ticketDbId }
                  : null,
              }
            : a,
        ),
      ...
      try {
        ...
        const [statsRes, ticketsRes] = await Promise.all([
          ...
          ticketService.getAll({
            limit: 6,
            myTickets: user?.role === "support_agent" ? "true" : undefined,
          }),
        ]);
        ...
        const generatedChartData = Array.from({ length: 7 }).map((_, i) => {
          ...
          const createdItem = trend.created?.find(
            (item) => item._id === dateStr,
          );
          const resolvedItem = trend.resolved?.find(
            (item) => item._id === dateStr,
          );
        ...
        const totalCreatedThisWeek = generatedChartData.reduce(
          (acc, curr) => acc + curr.tickets,
          0,
        );
        const totalResolvedThisWeek = generatedChartData.reduce(
      ...
      >
        ...
        >
          <div>
            ...
            <p style={{ opacity: 0.9 }}>
              Check the status of your requests or find answers in the knowledge
              base.
            </p>
          </div>
          <Button
        ...
        <div className="dashboard-grid" style={{ marginBottom: "var(--s-8)" }}>
          ...
          >
            ...
            >
              Resolved tickets waiting for your feedback.
            </p>
          </div>
          ...
          >
            ...
            >
              <div>
                ...
                >
                  Real-time stream of incoming and active requests
                </p>
              </div>
              <Button
                variant="ghost"
                              ...
                              >
                                <div
                                  style={{
                                    ...
                                    borderRadius: "50%",
                                    background:
                                      PRIORITY_COLORS[t.priority.toLowerCase()],
                                    boxShadow: `0 0 10px ${PRIORITY_COLORS[t.priority.toLowerCase()]}`,
                                  }}
                                />
                                <span
                                  style={{
  ...
  return (
    ...
    >
      {/* Hero Header */}
      <div
        className="flex-between mb-8 dashboard-hero"
        style={{
        ...
        <div>
          <h1 style={{ fontSize: "2rem", fontWeight: 800, color: "white" }}>
            {user?.role === "support_agent"
              ? `Ready for work, ${user?.name?.split(" ")[0] ?? "there"}?`
              : "Welcome back, Team!"}
          </h1>
          <p style={{ opacity: 0.85 }}>
            {user?.role === "support_agent"
              ? "Here is a summary of your assigned tasks and performance today."
              : "Here’s your support performance overview for today."}
          </p>
        </div>
      ...
      <div className="dashboard-grid-4" style={{ marginBottom: "var(--s-8)" }}>
        ...
        >
          ...
          >
            ...
            <span>{stats?.trendCreated || 0} this week</span>
          </div>
        </div>
        ...
        >
          ...
          >
            <ArrowUpRight size={14} />{" "}
            <span>{stats?.trendResolved || 0} this week</span>
          </div>
        </div>
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, color: "#92400e" }}>
              {stats.pendingReassignRequests} Ticket Reassignment Requests
              Pending
            </div>
            <div style={{ fontSize: "0.82rem", color: "#b45309" }}>
              Agents are asking to transfer their workload. Action required from
              Admin.
            </div>
          </div>
                      ...
                      >
                        ...
                        <td className="hide-mobile">
                          <div className="flex-center gap-2">
                            <span
                              className={`priority-indicator priority-${t.priority}`}
                            />
                            <span
                              style={{
                                textTransform: "capitalize",
                        ...
                        <td
                          className="hide-tablet hide-mobile"
                          style={{
                            color: "var(--text-dim)",
                            fontSize: "0.8rem",
                          }}
                  ...
                  >
                    <div
                      style={{
                        ...
                        borderRadius: "50%",
                        background:
                          PRIORITY_COLORS[p.name.toLowerCase()] ||
                          "var(--primary)",
                      }}
                    />
                ...
                <div className="flex-col gap-3">
                  {["morning", "mid", "night"].map((shift) => (
                    ...
                    >
                      ...
                      >
                        {stats?.shiftStats?.[shift] || 0} Tickets
                      </span>
                    </div>
                  ))}
                </div>
                    ...
                    <div>
                      {agent.liveStatus === "on_site" ? (
                        <div style={{ textAlign: "right" }}>
                          ...
                          >
                            {agent.onSiteTicket?.onSiteVisit
                              ?.arrivalConfirmedByEmployee
                              ? "✅ Verified"
                              : "⏳ Unverified"}
                          </div>
                        </div>
