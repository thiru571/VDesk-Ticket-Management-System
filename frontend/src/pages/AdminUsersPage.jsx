...
const ROLE_CONFIG = {
  ...
  support_agent: { label: 'Support Agent', bg: '#DBEAFE', color: '#1E40AF', dot: '#3B82F6' },
  employee:      { label: 'Employee',      bg: '#F3F4F6', color: '#374151', dot: '#9CA3AF' },
};
...
function WorkloadBar({ user }) {
  ...
  const label = isCritical ? 'Critical' : isHigh ? 'High Load' : `${Math.round(pct)}%`;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', minWidth: '80px' }}>
      ...
      <span style={{ fontSize: '10px', fontWeight: 700, color: barColor, letterSpacing: '0.02em' }}>
        {label}
      </span>
    </div>
  );
...
export default function AdminUsersPage() {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const res = await api.get('/users', { params: { search, role: roleFilter, page, limit: PAGE_LIMIT } });
  ...
  const handleCreate = async () => {
    if (!createForm.email || !createForm.name) return toast.error('Email and name are required');
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      toast.success(`${res.data.deleted} user${res.data.deleted !== 1 ? 's' : ''} deleted`);
      setSelectedIds([]);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="page-layout">
        ...
        {selectedIds.length > 0 && (
          <motion.div
            ...
            style={{
              ...
              marginBottom: '16px', boxShadow: '0 4px 20px rgba(31,78,121,0.3)'
            }}
          >
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => setSelectedIds([])}
                style={{ background: 'rgba(255,255,255,0.15)', border: 'none', color: 'white', padding: '6px 14px', borderRadius: '6px', cursor: 'pointer', fontWeight: 600, fontSize: '0.82rem' }}>
                Clear
              </button>
              <button onClick={() => setShowDeleteConfirm(true)}
                style={{ background: '#DC2626', border: 'none', color: 'white', padding: '6px 14px', borderRadius: '6px', cursor: 'pointer', fontWeight: 700, fontSize: '0.82rem' }}>
                Delete Selected
              </button>
            </div>
          </motion.div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-dark)', margin: 0, lineHeight: 1.2 }}>Team Management</h1>
          <p style={{ color: 'var(--text-dim)', fontSize: '0.875rem', marginTop: '4px', margin: '4px 0 0' }}>
            Manage users, roles, and access permissions
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          <button onClick={downloadTemplate}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', border: '1px solid var(--border)', background: 'white', borderRadius: '8px', cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600, color: 'var(--text-dim)' }}>
            <Download size={14} /> CSV Template
          </button>
          <button onClick={() => fileInputRef.current?.click()} disabled={importing}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', border: '1px solid var(--border)', background: 'white', borderRadius: '8px', cursor: 'pointer', fontSize: '0.82rem', fontWeight: 600, color: 'var(--text-dim)', opacity: importing ? 0.7 : 1 }}>
            <Upload size={14} /> {importing ? 'Importing…' : 'Import CSV'}
          </button>
          <input ref={fileInputRef} type="file" accept=".csv" style={{ display: 'none' }} onChange={handleBulkImport} />
          <button onClick={() => setIsCreateModalOpen(true)}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', background: '#1F4E79', border: 'none', color: 'white', borderRadius: '8px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 700 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flex: 1, flexWrap: 'wrap' }}>
          ...
          <div style={{ position: 'relative', minWidth: '220px', flex: 1, maxWidth: '340px' }}>
            <Search size={15} style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-dim)', pointerEvents: 'none' }} />
            <input
              placeholder="Search by name or email…"
              className="input"
              style={{ paddingLeft: '32px', height: '34px', fontSize: '0.82rem', width: '100%', background: 'var(--bg)', border: '1px solid transparent' }}
              value={search}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            {['', ...ROLES].map(r => (
              <button key={r} onClick={() => setRoleFilter(r)}
                ...
                {r === '' ? 'All' : r === 'support_agent' ? 'Support' : r.charAt(0).toUpperCase() + r.slice(1)}
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          {/* View toggle */}
          <div style={{ display: 'flex', background: 'var(--surface-alt)', borderRadius: '7px', padding: '3px', border: '1px solid var(--border)', gap: '2px' }}>
            {[
              { mode: 'list',  icon: <LayoutList size={14} /> },
              { mode: 'board', icon: <LayoutGrid size={14} /> },
            ].map(({ mode, icon }) => (
              <button key={mode} onClick={() => { setViewMode(mode); localStorage.setItem('userViewMode', mode); }}
                style={{
      ...
      {/* ── Board View ── */}
      {viewMode === 'board' ? (
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${ROLES.length}, 1fr)`, gap: '16px', minHeight: '60vh' }}>
          {ROLES.map(role => {
            ...
            return (
              <div key={role}
                ...
                onDrop={e => { e.preventDefault(); handleRoleChange(e.dataTransfer.getData('userId'), role); }}
                style={{ background: 'var(--surface-alt)', borderRadius: '12px', border: '1px solid var(--border)', padding: '14px', display: 'flex', flexDirection: 'column', gap: '10px' }}
              >
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                  <span style={{ fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.06em', color: cfg.color }}>{cfg.label}</span>
                  <span style={{ fontSize: '11px', fontWeight: 700, background: cfg.bg, color: cfg.color, padding: '2px 8px', borderRadius: '999px' }}>{roleUsers.length}</span>
                </div>
                {roleUsers.map(u => (
                  <motion.div key={u._id} draggable
                    onDragStart={e => { e.dataTransfer.setData('userId', u._id); e.target.style.opacity = '0.5'; }}
                    onDragEnd={e => { e.target.style.opacity = '1'; }}
                    onClick={() => openEdit(u)}
                    whileHover={{ y: -2 }}
                    style={{ background: 'white', padding: '10px 12px', borderRadius: '10px', border: '1px solid var(--border)', cursor: 'grab', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <Avatar name={u.name} size={30} />
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontWeight: 700, fontSize: '0.82rem', color: 'var(--text-dark)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{u.name}</div>
      ...
      /* ── List View / Table ── */
      <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
            ...
            <tr style={{ background: '#1F4E79' }}>
              <th style={{ padding: '12px 14px', textAlign: 'center' }}>
                <input type="checkbox" checked={allSelected} onChange={toggleAll}
                  style={{ width: '15px', height: '15px', accentColor: 'white', cursor: 'pointer', display: 'block', margin: '0 auto' }} />
              </th>
              {['User', 'Role', 'Department', 'Workload', 'Last Login', 'Status', ''].map((h, i) => (
                <th key={i} style={{
                  padding: '12px 14px', textAlign: h === 'Workload' || h === 'Status' || h === '' ? 'center' : 'left',
                ...
                ? Array.from({ length: 8 }).map((_, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #F3F4F6' }}>
                      <td colSpan={8} style={{ padding: '10px 14px' }}>
                        <div style={{ height: '36px', background: 'linear-gradient(90deg, #F3F4F6 25%, #E5E7EB 50%, #F3F4F6 75%)', backgroundSize: '400px 100%', borderRadius: '6px', animation: 'shimmer 1.4s infinite linear' }} />
                      </td>
                    </tr>
                  ))
                : users.map((u, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: '12px 14px', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
                          <input type="checkbox" checked={isSelected} onChange={() => toggleOne(u._id)}
                            style={{ width: '15px', height: '15px', accentColor: '#1F4E79', cursor: 'pointer', display: 'block', margin: '0 auto' }} />
                        </td>
                        ...
                        <td style={{ padding: '12px 14px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
                            ...
                            <div style={{ minWidth: 0 }}>
                              <div style={{ fontWeight: 700, fontSize: '0.875rem', color: 'var(--text-dark)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', lineHeight: 1.3 }}>{u.name || '—'}</div>
                              <div style={{ fontSize: '0.72rem', color: 'var(--text-dim)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: '1px' }}>{u.email}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        {/* Department */}
                        <td style={{ padding: '12px 14px', fontSize: '0.82rem', color: 'var(--text-main)', fontWeight: 500 }}>
                          {u.department || '—'}
                        </td>
                        ...
                        {/* Last Login */}
                        <td style={{ padding: '12px 14px', fontSize: '0.8rem', color: 'var(--text-dim)', whiteSpace: 'nowrap' }}>
                          {u.lastLogin ? timeAgo(u.lastLogin) : <span style={{ color: '#D1D5DB' }}>Never</span>}
                        </td>

                        {/* Status */}
                        <td style={{ padding: '12px 14px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', flexWrap: 'wrap' }}>
                            <StatusPill type={u.isVerified ? 'verified' : 'pending'} />
                            <StatusPill type={u.isActive ? 'active' : 'inactive'} />
                          </div>
                        </td>
                        ...
                        <td style={{ padding: '12px 14px', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
                          <button onClick={() => openEdit(u)}
                            style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-dim)', padding: '4px 6px', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.1s' }}
        ...
        {!loading && users.length === 0 && (
          <div style={{ padding: '60px 24px', textAlign: 'center', color: 'var(--text-dim)' }}>
            ...
            <div style={{ fontWeight: 600, fontSize: '0.9rem' }}>No users found</div>
            <div style={{ fontSize: '0.8rem', marginTop: '4px' }}>Try adjusting your search or filters</div>
          </div>
        )}
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <button onClick={() => setPage(p => p - 1)} disabled={page <= 1}
              style={{ width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '6px', border: '1px solid var(--border)', background: 'white', cursor: page <= 1 ? 'not-allowed' : 'pointer', opacity: page <= 1 ? 0.4 : 1 }}>
              <ChevronLeft size={14} />
            </button>
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(p => (
              <button key={p} onClick={() => setPage(p)}
                style={{ width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '6px', border: '1px solid', fontSize: '12px', fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s',
            ...
            {totalPages > 5 && <span style={{ fontSize: '12px', color: 'var(--text-dim)', padding: '0 2px' }}>…</span>}
            <button onClick={() => setPage(p => p + 1)} disabled={page >= totalPages}
              style={{ width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '6px', border: '1px solid var(--border)', background: 'white', cursor: page >= totalPages ? 'not-allowed' : 'pointer', opacity: page >= totalPages ? 0.4 : 1 }}>
      ...
      >
        <p style={{ fontSize: '0.95rem', fontWeight: 500, color: 'var(--text-main)' }}>
          Change <strong>{roleConfirm.userName}</strong>'s role to <strong>{roleConfirm.newRole?.replace('_', ' ')}</strong>?
        </p>
      </Modal>
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header flex-between">
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  <h3 className="card-title" style={{ marginBottom: 0 }}>{selectedUser.name}</h3>
                  <p style={{ fontSize: '0.75rem', color: 'var(--text-dim)', margin: 0 }}>{selectedUser.email}</p>
                </div>
              </div>
              ...
              <div className="flex-col gap-6">
                ...
                <div className="flex-between p-4" style={{ background: 'var(--bg)', borderRadius: 'var(--r-md)' }}>
                  ...
                  <button onClick={() => setEditForm({...editForm, isActive: !editForm.isActive})}
                    style={{ width: '44px', height: '22px', background: editForm.isActive ? '#10B981' : 'var(--text-dim)', borderRadius: 'var(--r-full)', position: 'relative', transition: 'var(--t-fast)', border: 'none', cursor: 'pointer' }}>
        ...
        <div className="modal-overlay" onClick={() => setIsResetModalOpen(false)}>
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '420px' }}>
            <div className="modal-header">
              ...
              <p style={{ fontSize: '0.8rem', color: 'var(--text-dim)' }}>New password for {selectedUser.name}.</p>
            </div>
            <div className="modal-body">
              <div className="input-group">
        ...
        <div className="modal-overlay" onClick={() => { setIsCreateModalOpen(false); setCreatedPassword(null); }}>
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <p style={{ fontSize: '0.8rem', color: 'var(--text-dim)' }}>Credentials are stored securely.</p>
            </div>
            {createdPassword ? (
              <div className="modal-body">
                <div style={{ background: '#F0FDF4', border: '1.5px solid #86EFAC', borderRadius: '10px', padding: '20px', textAlign: 'center' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'white', border: '1.5px solid #1F4E79', borderRadius: '8px', padding: '10px 14px' }}>
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(createdPassword); toast.success('Copied!'); }}
                      style={{ background: '#1F4E79', color: 'white', border: 'none', borderRadius: '6px', padding: '6px 14px', cursor: 'pointer', fontWeight: 700, fontSize: '0.82rem' }}>
                      Copy
                    </button>
                  </div>
                </div>
                  ...
                  <div className="input-group">
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      ...
                      <button type="button" onClick={() => setCreateForm({...createForm, password: generatePassword()})}
                        style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', padding: '0 12px', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 700, color: 'var(--primary)', whiteSpace: 'nowrap' }}>
                        Generate
                      </button>
                    </div>
                  </div>
      ...
      {showDeleteConfirm && (
        <div className="modal-overlay" onClick={() => setShowDeleteConfirm(false)}>
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '440px' }}>
            <div className="modal-header">
              <h3 className="card-title" style={{ color: '#DC2626' }}>Delete Users</h3>
            </div>
            <div className="modal-body">
              <p style={{ fontSize: '0.95rem', lineHeight: 1.6, color: 'var(--text-main)' }}>
                Permanently delete <strong style={{ color: '#DC2626' }}>{selectedIds.length} user{selectedIds.length !== 1 ? 's' : ''}</strong>?
                <br /><span style={{ fontSize: '0.85rem', color: 'var(--text-dim)' }}>This action cannot be undone.</span>
              </p>
            </div>
            <div className="modal-footer">
              ...
              <button onClick={handleBulkDelete} disabled={deleting}
                style={{ background: '#DC2626', color: 'white', border: 'none', padding: '8px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: 700, opacity: deleting ? 0.7 : 1 }}>
